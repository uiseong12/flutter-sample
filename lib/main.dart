...
class _GameShellState extends State<GameShell> with TickerProviderStateMixin {
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [const Color(0xCC140F22), const Color(0x99140F22), const Color(0x00140F22)],
              ),
              border: const Border(bottom: BorderSide(color: Color(0x55FFFFFF))),
            ),
            child: Row(
              children: [
                _currencyChip(
  ...
  Widget build(BuildContext context) {
    ...
    final contentTopInset = _menuIndex == 0 ? 0.0 : (_hudTooltipText != null ? 104.0 : 60.0);
  ...
  Widget _branchRouteMap({required double height}) {
    ...
    final maxChapter = _story.length;

    int frontier = _storySelections.lastIndexWhere((e) => e != null) + 1;
    if (frontier < 1) frontier = 1;
    if (frontier > maxChapter) frontier = maxChapter;

    final nodes = <Map<String, dynamic>>[];
    final edges = <Map<String, dynamic>>[];

    double jitter(String seed, double amp) {
      final h = seed.hashCode.abs() % 1000;
